...
using PerfCds.CtfExtensions.DescriptorInterfaces;

namespace PerfCds.CtfExtensions.Descriptors
{
    /// <summary>
        ...
        : IEventDescriptor
    {
                ...
                IReadOnlyDictionary<string, ICtfTypeDescriptor> typeDeclarations)
        {
            ...
            {
                throw new PerfMetadataException("event descriptor is missing 'name' property.");
            }
        ...
        private void SetId(IReadOnlyDictionary<string, string> assignments)
        {
            ...
            {
                throw new PerfMetadataException("event descriptor is missing 'id' property.");
            }

            if (!uint.TryParse(integerString, out var id))
            {
                throw new PerfMetadataException($"event descriptor 'id' cannot be converted into a {this.Id.GetType()}.");
            }
        ...
        private void SetStream(IReadOnlyDictionary<string, string> assignments)
        {
            ...
            {
                throw new PerfMetadataException("event descriptor is missing 'stream_id' property.");
            }

            if (!int.TryParse(integerString, out var streamId))
            {
                throw new PerfMetadataException($"event descriptor 'stream_id' cannot be converted into a {this.Stream.GetType()}.");
            }
        ...
        private void SetLogLevel(IReadOnlyDictionary<string, string> assignments)
        {
            ...
            {
                throw new PerfMetadataException($"event descriptor 'loglevel' cannot be converted into a {this.Stream.GetType()}.");
            }
        ...
        private void SetPayload(IReadOnlyDictionary<string, ICtfTypeDescriptor> typeDeclarations)
        {
            ...
            {
                throw new PerfMetadataException("event descriptor is missing 'fields' type assignment.");
            }

            if (!(typeDescriptor is ICtfStructDescriptor payloadStruct))
            {
                throw new PerfMetadataException("event descriptor type 'fields' is not a structure.");
            }
